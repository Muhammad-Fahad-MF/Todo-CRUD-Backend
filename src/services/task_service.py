...
from src.models.task_models import Stats, Task, TaskCreate, TaskPatch, TaskUpdate


def get_all_tasks(done: bool | None, search: str | None) -> list[Task]:
    """Business layer to retrieve all tasks with optional query filters."""
    return db_get_all(done=done, search=search)


def get_task_by_id(task_id: int) -> Task | None:
    """Business layer to retrieve a single task by ID."""
    return db_get_by_id(task_id)
